    ...
    $.login = function (text, title, onOK, onCancel, username, password) {
        ...
        var modal = $.modal({
            text: '<p class="weui-prompt-text">' + (config.text || '') + '</p>' +
                '<input type="text" class="weui-input weui-prompt-input" id="weui-prompt-username" value="' + (config.username || '') + '" placeholder="输入账号" />' +
...
+function ($) {
    ...
    $.toast = function (text, style, callback) {
        ...
        show('<i class="' + iconClassName + ' weui-icon_toast"></i><p class="weui-toast_content" style="margin-top: 1em;">' + (text || "已经完成") + '</p>', className);
